...
Map<K, V> uniffiNormalizeU64Map<IK, IV, K, V>(
  Map<IK, IV> input,
  K Function(IK) normalizeKey,
  V Function(IV) normalizeValue,
) {
  final output = <K, V>{};
  for (final entry in input.entries) {
    ...
    if (output.containsKey(key)) {
      throw ArgumentError(
        "Duplicate map key after u64 conversion: " + key.toString(),
      );
    }
...
final uniffiForeignFutureHandleMap =
    UniffiHandleMap<UniffiForeignFutureState>();
...
uniffiForeignFutureFreePointer = Pointer.fromFunction<UniffiForeignFutureFree>(
